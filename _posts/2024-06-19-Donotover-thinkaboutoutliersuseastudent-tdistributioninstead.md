...
pp_check(Gaussian_Fit1, (ndraws = 100)) + labs((title = "가우시안 모델")) + theme_classic();
```
...
```js
describe_posterior(Gaussian_Fit1, (centrality = "mean"), (dispersion = TRUE), (ci_method = "HDI"), (test = "rope"));
```
...
```js
describe_posterior(Student_Fit, (centrality = "mean"), (dispersion = TRUE), (ci_method = "HDI"), (test = "rope"));
```
...
```js
Student_CondEffects < -conditional_effects(Student_Fit);

Student_CondEffects < -plot(Student_CondEffects, (plot = FALSE))[[3]];
...
```js
Student_CondEffects2 < -conditional_effects(Student_Fit2);

Student_CondEffects2 < -plot(Student_CondEffects2, (plot = FALSE))[[3]];
...
Student_CondEffects3 < -conditional_effects(Student_Fit2, (dpar = "sigma"));

Student_CondEffects3 < -plot(Student_CondEffects3, (plot = FALSE))[[3]];

Student_CondEffects3 + Plot_theme + theme((legend.position = "bottom"), (legend.direction = "horizontal"));
```
...
```js
Fit_Comp < -compare_performance(Gaussian_Fit1, Student_Fit, Student_Fit2, (metrics = "all"));
